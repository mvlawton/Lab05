...
public class PokerTableController {
	...
	private void handlePlay() {
		...
		/* first attempt
		if (MainApp.iGameType == 1){
			Rule rle = new Rule(eGame.FiveStud);
			gme = new GamePlay(rle);
		}
		else if (MainApp.iGameType == 5){
			Rule rle1 = new Rule(eGame.Omaha);
			gme = new GamePlay(rle1);
		}
		else if (MainApp.iGameType == 4){
			Rule rle2 = new Rule(eGame.TexasHoldEm);
			gme = new GamePlay(rle2);
		}
		// Get the Rule, start the Game
		String game = "FiveStud"; //need to find way to retrieve from choice in menu
		...
		case "TexasHoldEm":
			...
			gme = new GamePlay(rle);
		case "DeucesWild":
			Rule rle1 = new Rule(eGame.DeucesWild);
			gme = new GamePlay(rle1);
		
		case "FiveStud":
			Rule rle2 = new Rule(eGame.FiveStud);
			gme = new GamePlay(rle2);
		
		case "Omaha":
			Rule rle3 = new Rule(eGame.Omaha);
			gme = new GamePlay(rle3);
	...
	private void handleDraw() {
		...
		if (MainApp.iGameType == 1) { //five stud draw
		...
		else if (MainApp.iGameType == 4){ //hold em draw
			for (Player p : mainApp.GetSeatedPlayers()) {
				Card c = gme.getGameDeck().drawFromDeck();
